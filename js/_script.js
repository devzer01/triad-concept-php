...
var root_path = 'http://www.yourbuddy24.com/'; //'http://server/yourbuddy24.com/'; //

		
function addFavorite(originalRequest)
{
...
function removeFavorite(originalRequest)
{
	if(originalRequest.status == 200)
	{	
		if(originalRequest.responseText == 1)
...
function listFavorite(originalRequest)
{
	if(originalRequest.status == 200)
		$('listfavorite').innerHTML = originalRequest.responseText;
}
...
function checkNullSignup1()
{
	...
	/*var data = Array(Array('username', username, '==', '', username_alert), 
					 Array('username', username.length, '<', 6, usernameLength_alert), 
					 Array('username', username.length, '>', 9, usernameLength_alert), 
					 Array('password', password, '==', '', password_alert), 
					 Array('password', password.length, '<', 6, passwordLength_alert), 
					 Array('password', password.length, '>', 9, passwordLength_alert), 
...
function checkUsernameSilent(username){
	if(checkNull(username)){
		if(username==old_username){
			...
			if((username.length>=6) && (username.length<=30)){
				if(checkFormUserName(username)){
					...
					$("username_info").innerHTML = username_invalid_alert;
				}	
			}
			else{
				//$("username_indicator").innerHTML="X";
				$("username_info").innerHTML = usernameLength_alert;
				username_ok = false;
			}
		}
	}
		...
		//$("username_indicator").innerHTML="X";
		$("username_info").innerHTML = can_not_empty;
		username_ok = false;
	}
}
...
function checkUsernameSilentResult(originalRequest){
	if(originalRequest.responseText==0){
		...
		$("username_info").innerHTML = duplicate_nickname;
		username_ok = false;
	}
	hideWaitingBox();
}
...
function checkEmailSilent(email)
{
	if(checkNull(email))
	{
		...
		{
			$("email_info").innerHTML = emailForm_alert;
		}
	}
	else
	{
		$("email_info").innerHTML = email_alert;
	}
}
...
function checkEmailSilentResult(originalRequest)
{
	if(originalRequest.responseText==0)
		...
		$("email_info").innerHTML = duplicate_email;
}
...
function checkNullPassword(password)
{
	if((password.length>=6) && (password.length<=30))
		...
		$("password_info").innerHTML = passwordLength_alert;
}
...
function checkMatching(fstVal,sndVal)
{
	if(fstVal == sndVal)
		...
		$("confirm_password_info").innerHTML = confirmpasswordMatch_alert;
}
...
function checkAcept(acept)
{
	if(acept.checked !== false)
		...
		$("accept_info").innerHTML = accept_alert;
}
		...
		var mbox = new Ajax.Request("ajaxRequest.php", 
									{
										...
										onComplete: function(originalRequest) {
											if((originalRequest.responseText==="Step2") || (originalRequest.responseText==="Step3"))
											{
												...
												//GB_show("", root_path + "?action=incompleteinfo", 345, 730);
												Lightview.show({
																href: popup_url,
																rel: 'ajax',
																options: {
																	...
																	ajax: {
																		onComplete: function() {
																			...
																			//$('register_form').observe('submit', submitAjaxFormDemonstration);
																			//return false;
		...
		var mbox = new Ajax.Request("ajaxRequest.php", 
									{
										...
										onComplete: function(originalRequest) {
											...
											{
												Lightview.show({
																href: root_path + '?action=mobileverify',
																rel: 'ajax',
																options: {
																	autosize: true,
																	topclose: true/*,
		...
		var mbox = new Ajax.Request("ajaxRequest.php", 
									{
										...
										onComplete: function(originalRequest) {
											...
											{
												Lightview.show({
																href: root_path + '?action=mobileverify_successful',
																rel: 'ajax',
																options: {
																	autosize: true,
																	topclose: true/*,
												...
												$('mobile_ver_code_info').innerHTML = originalRequest.responseText;
										}
									});
	}
}
	...
	var mbox = new Ajax.Request("ajaxRequest.php", 
									{
										...
										onComplete: function(originalRequest) {
											...
											{
												Lightview.show({
																href: root_path + '?action=incompleteinfo',
																rel: 'ajax',
																options: {
																	autosize: true,
																	topclose: true/*,
	...
	var mbox = new Ajax.Request("ajaxRequest.php", 
								{
									...
									onComplete: function(originalRequest) {
										if(originalRequest.responseText!="")
										{
											$('resend_code_info').innerHTML = originalRequest.responseText;
										}
									}
								});
								return false;
