...
function smarty_function_paginate_middle($params, &$smarty) {
    ...
    if(isset($_page_limit)) {
		...
		if( ($_view = $_total-$_item_start )< $_page_limit) {
			$_item = $_total - $_page_limit + 1;
        }
		$_item = ($_item >= 1) ? $_item : 1;
	...
	while($_item <= $_total) {
        ...
        if($_item != $_curr_item) {
        	...
        	if(empty($_current_prefix)) {
            	$_ret .= $_link_prefix . '<span class="active">'.$_text.'</span>' . $_link_suffix;
			} else {
