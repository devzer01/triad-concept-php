...
//{
	...
	if($skip_step=="Verified")
		header('Location: .');
	elseif($skip_step=="Step2")
		header('Location:?action=incompleteinfo_skip');
	...
	if(isset($_POST['submit_hidden'])){
			...
			elseif(strlen(trim($_POST['mobile_ver_code'])) > 0)
			{
				switch(funcs::verifyMobile($_SESSION['sess_username'],$_POST['mobile_ver_code'])){
					case 1://complete comfirm
						...
						$subject = funcs::getText($_SESSION['lang'], '$phone_number_guide_subject');	//get subject message
						$message = funcs::getMessageEmail_mobileGuide($smarty, $_SESSION['sess_username']);
						$userid = funcs::getUserid($_SESSION['sess_username']);
						$inbox_msg = funcs::getText($_SESSION['lang'], '$phone_number_guide');
						funcs::addMessageToInbox($userid, $subject, $inbox_msg);
						funcs::sendMail(funcs::getEmail($userid), $subject, $message, MAIL_FROM);	//send message to email
						if(isset($_SESSION['mobileverify_redirect']))
							header("location:".$_SESSION['mobileverify_redirect']); //go to first page
						else
							header("location: ./");
						exit();
					break;
		...
		{
			funcs::setNulCurrentUserMobileNo();
			header('Location:?action=incompleteinfo_skip');
			exit();	
	...
	//$smarty->display('mobileverify_skip.tpl');
