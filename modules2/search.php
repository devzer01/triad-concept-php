		...
		case 'searchGender':
			...
			if($_GET['type'] == "searchMembers")
			{
				...
				do
				{
					...
					{
						$start = $next;
						$num = SEARCH_RESULTS_PER_PAGE-count($id_arr);

						$profileDetails = search::geProfile('', $_GET['q_gender'], '', $_GET['q_picture'], $_GET['country'], $_GET['state'], $_GET['city'], $_GET['q_minage'], $_GET['q_maxage'], $start, $num, $id_arr);

						if(isset($profileDetails['datas']))
							$result = array_merge($result, $profileDetails['datas']);
						if(isset($profileDetails['total']))
						{
							if($previousTotal < $profileDetails['total'])
								$memrec_total += $profileDetails['total'];

							$previousTotal = $profileDetails['total'];
						}
					}
					$last_search_hash = $search_hash;
				...
				//while((count($result)<SEARCH_RESULTS_PER_PAGE) && ($memrec_total<(2*SEARCH_RESULTS_PER_PAGE)));
				while($memrec_total<2);*/
