      ...
      <tr>
        <td>
          <table border=0>
			...
			<tr><td class="text12grey" colspan="3" align="center">{$payment_text}</td></tr>
            <tr>
              <td valign="top" class="text14grey">
                <font face="Arial,Helvetica">
                   Bankname
...
<tr>
	<td colspan="3" class="text14grey">Dein derzeitiger Mitgliedsstatus:</td>
</tr>
<tr bgcolor="#0099FF" bordercolor="#FFFFFF" height="20px" style="color:#FFFFFF" valign="middle">
	<th>Derzeitige Mitgliedsstufe</th>
	{if $payment_history.id}<th>Seit</th>
	<th>Gültig bis</th>
	<th>#</th>{/if}
</tr>
<tr bgcolor="#eeeeee">
	<td align="center">{$payment_history.type}</td>
	{if $payment_history.id}<td align="center">{$payment_history.start_date}</td>
	<td align="center">{$payment_history.end_date}</td>
	<td align="center">
		{if $payment_history.cancelled eq 0}
		<span style="color: red; cursor: pointer" onclick="if(confirm('Möchtest du deine Mitgliedschaft tatsächlich beenden?')) location='?action=membership&type=cancel&id={$payment_history.id}'">Cancel this membership payment.</span>
		{else}
