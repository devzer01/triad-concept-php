...
$path = $_SERVER['DOCUMENT_ROOT'].dirname($_SERVER['PHP_SELF'])."/".SITE;
if($package['currency']=="GBP")
	$config=$path."paysafecard/merchant_direct_gbp.properties";
elseif($package['currency']=="EUR")
	$config=$path."paysafecard/merchant_direct_eur.properties";
else
	die($package['currency']." is not supported!");
$okurl=$url."?action=payment_response_paysafecard";
$nokurl=$url."?action=payment_failed";
$pnurl=$url."";
$globaldebug="0";
...
function create_disposition($mid, $mtid, $merchantClientId, $amount, $currency, $okurl, $nokurl, $pnurl, $config, $businesstype, $reportingcriteria)
{
   global $globaldebug;
   global $error_message;
...
/* requrl: the URL executed later on */
   ...
   $reqparam = "currency=$currency&mid=$mid&mtid=$mtid&merchantClientId=$merchantClientId&amount=$amount&businesstype=$businesstype&reportingcriteria=$reportingcriteria&okurl=$okurl&nokurl=$nokurl&pnurl=$pnurl&language=$language";
