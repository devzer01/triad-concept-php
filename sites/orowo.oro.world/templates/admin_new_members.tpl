...
<div style="margin-top:10px;">
	...
	<tr>
		<td>
			<a href="#" onclick="checkWithChatServer('', 'checkAllWithChatServer')" class="admin-link">Check all with chat server</a>
		</td>
	</tr>
	{/if}
	<tr>
			...
			<tr  bgcolor="{cycle values="#ccb691,#fde6be"}" height="28px">
					...
					<td align="center">
						{if $userdata.isactive}
							<a href="?action=admin_view_messages&username={$userdata.username}" class="admin-link">Messages</a>
						...
						{else}
							<a href='#' class='emailcheck' data-id='{$userdata.user_id}' data-email='{$userdata.email}'>Check Email</a>
							<span id='email_status_{$userdata.user_id}'></span>
						{/if}
					</td>
				</tr>
			{/foreach}
...
jQuery(function () {
	jQuery(".emailcheck").click(function (e) {
		e.preventDefault();
		...
		jQuery.ajax(
				{
					...
					dataType: 'json',
					beforeSend: function () {
						jQuery('#email_status_' + user_id).html('<img height=16 width=16 src="images/ajax/animation.gif" />');
					},
					success: function (json) {
						if (json.status==0) {
							jQuery('#email_status_' + user_id).html('<img height=16 width=16 src="images/ajax/invalid.png" />');
						} else {
							jQuery('#email_status_' + user_id).html('<img height=16 width=16 src="images/ajax/valid.png" />');
						}
					}
				}
		);
